...
import com.amazon.tv.leanbacklauncher.CleanDeviceMemory;
...
public class LaunchPointListGenerator {
    ...
    private class CreateLaunchPointListTask extends AsyncTask<Void, Void, List<LaunchPoint>> {
        ...
        protected List<LaunchPoint> doInBackground(Void... params) {
            ...
            //Intent intent = LaunchPointListGenerator.this.mContext.getPackageManager().getLaunchIntentForPackage("com.android.traceur");
            //LaunchPoint lp = new LaunchPoint(LaunchPointListGenerator.this.mContext, mContext.getString(R.string.launcher_settings), mContext.getDrawable(R.drawable.ic_settings_launcher), intent, 0);
            //lp.setPriority(-4);
            //launcherItems.add(lp);
    ...
    private ArrayList<LaunchPoint> createSettingsList() {
        ...
        Intent intent_custom = new Intent(this.mContext, CleanDeviceMemory.class);
        intent_custom.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
        intent_custom.addFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION);
        intent_custom.addFlags(Intent.FLAG_ACTIVITY_NO_USER_ACTION);
        intent_custom.addFlags(Intent.FLAG_ACTIVITY_TASK_ON_HOME);
        ...
        for (int ptr = 0, size = rawLaunchPoints.size(); ptr < size; ptr++) {
            if(ptr == 1) continue;
            Log.d("RawLaunchPointsSettings", rawLaunchPoints.get(ptr).toString());
            ...
            if (info.activityInfo != null) {
                ...
                Log.d("addSettingsLaunchPoint",lp.getPackageName());
                if(lp.getPackageName().startsWith("com.android.tv.settings")) {
                    lp.setIconDrawable(mContext.getDrawable(R.drawable.ic_settings_settings));
                }
                // lp.setPriority(0);
        ...
        LaunchPoint lp = new LaunchPoint(this.mContext, mContext.getString(R.string.launcher_settings), mContext.getDrawable(R.drawable.ic_launcher), intent, 0);
